...
export const loadUser = () => async (dispatch) => {
  try {
    ...
    dispatch(loadUserFail(error.response?.data?.message || "User load failed"));
  }
};
...
export const updateProfile = (userData) => async (dispatch) => {
  try {
    ...
    const { data } = await api.put("/api/v1/update", userData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
...
export const updatePassword = (formData) => async (dispatch) => {
  try {
    ...
    await api.put("/api/v1/password/change", formData);
...
export const resetPassword = (formData, token) => async (dispatch) => {
  try {
    ...
    dispatch(
      resetPasswordFail(error.response?.data?.message || "Reset failed"),
    );
  }
};
...
export const updateUser = (id, formData) => async (dispatch) => {
  try {
    ...
    await api.put(`/api/v1/admin/user/${id}`, formData, {
      headers: { "Content-Type": "application/json" },
    });
